...
def skychart(time=None, location=None, show=False):
    ...
    # moon
    l_p, s_p = draw_moon_logo(az_frame, moon, sun, loc_gcrs,zoom_factor=15, resolution=40)
    ax.add_patch(l_p)
    ax.add_patch(s_p)

    # zenith
    # the marker '+' looks ugly in polar projection
    ax.plot([0,np.pi],[0.04,0.043], c=dark_yellow, lw=0.5,zorder=0)
    ax.plot([np.pi/2,np.pi/2*3],[0.04,0.044], c=dark_yellow, lw=0.5,zorder=0)
